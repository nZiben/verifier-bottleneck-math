...
class RayPRIMETrainer(RayPPOTrainer):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    # if not self.config.data.keep_bsz_after_filtering:
                    #     # remove placeholder batch after adv calculation
                    #     # (we should ensure the correctness of adv while avoid updating policy on these batches)
                    #     print(f"Filtering {int(sum(batch.non_tensor_batch['is_valid_input'].nonzero()))}/{len(batch)} rollouts")
                    #     batch = batch.select_idxs(batch.non_tensor_batch["is_valid_input"].nonzero()[0])
    ...
    def filter_and_downsample(self, scores, batch: DataProto):
        ...
        # batch.non_tensor_batch["is_valid_input"] = ~filter_mask.repeat(self.config.actor_rollout_ref.rollout.n).numpy()
