...
class DataProto:
    ...
    def check_consistency(self):
        ...
        if self.batch is not None and len(self.non_tensor_batch) != 0:
            ...
            for key, val in self.non_tensor_batch.items():
                assert isinstance(
                    val, np.ndarray
                ), 'data in the non_tensor_batch must be a numpy.array'
                assert val.shape[
    ...
    def select_idxs(self, idxs):
        ...
        if self.batch is not None:
            ...
            try:
                ...
                print(idxs_torch)
                print(idxs_torch.shape)
                print([f"{key}: {tensor.shape}" for key, tensor in self.batch.items()])
                exit()
        else:
            selected_batch = None
    ...
    def slice_with_index(self, index):
