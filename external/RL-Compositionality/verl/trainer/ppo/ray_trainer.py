...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        self.dynamic_selection_step_cnt = None
        self.last_dynamic_strategy = None
    ...
    def _create_dataloader(self):
        ...
        self.train_dataset = RLHFDataset(parquet_files=self.config.data.train_files,
                                         ...
                                         zero=self.config.data.zero,
                                         zero_template=self.config.data.zero_template)
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_gen_batch.meta_info = {
                ...
                'validate': True,
                'stop': self.config.actor_rollout_ref.rollout.val_kwargs.stop,
            }
            ...
            reward_tensor, acc_tensor = self.val_reward_fn(test_batch)
    ...
    def add_to_buffer(self, batch, batch_size, n_samples):
        ...
        batch = batch.slice_with_index(buffer_mask)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        gen_batch.meta_info['stop'] = self.config.actor_rollout_ref.rollout.get('stop', None)
                    ...
                    with _timer('reward', timing_raw):
                        ...
                        reward_tensor, acc_tensor = self.reward_fn(new_batch)

                        new_batch.batch['token_level_scores'] = reward_tensor
                        new_batch.batch['acc'] = acc_tensor
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        if self.config.data.keep_bsz_after_filtering:
                            if batch is None:
                                batch = new_batch
                            else:
                            ...
                            # if we don't keep batch size, we just use the new batch
                            batch = new_batch
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        self.last_entropy = metrics['actor/entropy_loss']
                ...
                num_prompt_in_batch = 0
                num_gen_batches = 0
                self.last_clip_ratio = metrics["response_length/clip_ratio"]
